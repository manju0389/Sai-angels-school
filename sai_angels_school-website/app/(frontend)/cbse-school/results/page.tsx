...
/**
 * Fetch the Result PDF details
 */
async function getThreeYearResultPDF() {
  const res = await fetch(
    `${apiUrl}/cbse-documents/pdf/results`,
    {
      cache: "no-store",
    }
  );
...
export default async function ThreeYearResultPage() {

  // Fetch PDF information from the API
  const pdf = await getThreeYearResultPDF();
    ...
    <main>
      ...
      <section className="w-full bg-gradient-to-r from-pink-300 via-purple-200 to-blue-300 py-10 px-6 mb-[4%]">
        <div className="mx-auto text-center space-y-10">
            <h1 className="text-4xl font-bold text-black-800 mb-3"> Three Year Result </h1>
        </div>
        </section>
