...
/**
 * Fetch the Building Certificate PDF details
 */
async function getBuildingCertificatePDF() {
  const res = await fetch(
    `${apiUrl}/cbse-documents/pdf/building-certificate`,
    {
      cache: "no-store",
    }
  );
...
export default async function BuildingCertificatePage() {
  ...
  const pdf = await getBuildingCertificatePDF();
    ...
    <main>
      ...
      <section className="w-full bg-gradient-to-r from-pink-300 via-purple-200 to-blue-300 py-10 px-6 mb-[4%]">
        <div className="mx-auto text-center space-y-10">
            <h1 className="text-4xl font-bold text-black-800 mb-3"> Building Certificate </h1>
        </div>
        </section>
