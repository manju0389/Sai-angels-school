...
/**
 * Fetch the Trust Certificate PDF details
 */
async function getTrustCertificatePDF() {
  const res = await fetch(
    `${apiUrl}/cbse-documents/pdf/society-trust-certificate`,
    {
      cache: "no-store",
    }
  );
...
/** Trust Certificate Page */
export default async function TrustCertificatePage() {

  // Fetch PDF information from the API
  const pdf = await getTrustCertificatePDF();
    ...
    <main>
      ...
      <section className="w-full bg-gradient-to-r from-pink-300 via-purple-200 to-blue-300 py-10 px-6 mb-[4%]">
        <div className="mx-auto text-center space-y-10">
            <h1 className="text-4xl font-bold text-black-800 mb-3"> Societies / Trust Certificate </h1>
        </div>
        </section>
          ...
          {pdfLink ? (
            <Link href={`${apiUrl}/cbse-documents/view/${pdf._id}`} target="_blank" rel="noopener noreferrer"
              ...
              <div className="flex items-center gap-3 rounded-md border-2 border-dashed border-white px-6 py-3 text-white font-semibold">
                ...
                <div>
                  {/* PDF Title */}
                  <div>Societies Trust Certificate</div>
