...
// Image type used for uploaded images and preview handling
type ImageItem = {
  url: string;
  public_id?: string;
  file?: File;
...
// Founder data structure received from API
type Founder = {
  _id: string;
  name: string;
  designation: string;
...
export default function Founder() {
  ...
  // API endpoint for founder CRUD operations
  const API = `${process.env.NEXT_PUBLIC_API_URL}/about-us/our-founder`;

  // Add founder form states
  const [name, setName] = useState("");
  const [designation, setDesignation] = useState("");
  const [description, setDescription] = useState("");

  // Selected files and their preview URLs
  ...
  // Temporary states used while editing founder details
  const [tempName, setTempName] = useState("");
  const [tempDesignation, setTempDesignation] = useState("");
  const [tempDescription, setTempDescription] = useState("");

  // Temporary images while editing
  const [tempImages, setTempImages] = useState<ImageItem[]>([]);

  // Fetch founder data when component loads
  useEffect(() => {
    fetchData();
  }, []);

  // Get all founder records from API
  ...
  // Handle image selection for new founder upload
